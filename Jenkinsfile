         ...
         script {
              withCredentials([usernamePassword(credentialsId: 'luis_kiuwan',
                        ...
                        usernameVariable: 'USERNAME')]) {
                         def returnCode = sh(returnStatus: true, script:'${AGENT_HOME}/bin/agent.sh -s \"${WORKSPACE}\" -n \"Blueblog\" -cr \"${BUILD_TAG}\" -l \"${BUILD_TAG}\" -bn \"${env.BRANCH_NAME}\" -wr --user \"$USERNAME\" --pass \"$PASSWORD\"')
